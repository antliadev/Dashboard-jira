...
export async function fetchIssuesFromDatabase(filters = {}) {
  if (!supabase) {
    console.error('[DB] fetchIssuesFromDatabase: Cliente Supabase não inicializado.');
    return [];
  }
...
export async function countIssuesInDatabase() {
  if (!supabase) {
    console.error('[DB] countIssuesInDatabase: Cliente Supabase não inicializado.');
    return 0;
  }
  ...
  if (error) {
    console.error(`[DB] countIssuesInDatabase Erro: ${error.message}`);
    return 0;
  }
...
export async function testJiraConnection(baseUrl, email, token, jql) {
  try {
    ...
    try {
      const countRes = await fetch(`${baseUrl}/rest/api/3/search/approximate-count`, {
        method: 'POST',
        headers: { 'Authorization': authHeader, 'Accept': 'application/json', 'Content-Type': 'application/json' },
        body: JSON.stringify({ jql: effectiveJql }),
        signal: AbortSignal.timeout(15000)
      });
      if (countRes.ok) {
        const countData = await countRes.json();
        totalTickets = countData.count || countData.total || totalTickets;
      }
    } catch (countError) {
