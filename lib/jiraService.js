...
class JiraService {
  ...
  async testConnectionWithConfig(baseUrl, email, token, jql = null) {
    try {
      // Validar inputs
      if (!baseUrl) {
        baseUrl = configService.getConfig().baseUrl;
        email = configService.getConfig().email;
        token = configService.getToken();
      }
      
      if (!baseUrl || !email || !token) {
        return { success: false, error: 'Credenciais não fornecidas' };
      }
      ...
      // Testa a JQL se fornecida
      let testResult = { total: 0, jqlValid: true };
      if (jql) {
        const jqlToTest = jql || configService.getJQL();
        ...
        if (jqlResponse.ok) {
          testResult = await jqlResponse.json();
        }
      }
  ...
  async fetchAllIssuesWithConfig(baseUrl, email, token, jql = null) {
    // Usa credenciais fornecidas ou do configService
    const config = baseUrl ? { baseUrl, email, token, jql } : configService.getConfig();
    
    if (!config.baseUrl || !config.email || !config.token) {
      throw new Error('Credenciais não fornecidas');
    }
    
    const effectiveBaseUrl = config.baseUrl.replace(/\/$/, '');
    const effectiveJql = jql || config.jql || configService.getJQL();
    
    // Não usa cache para requisições com credenciais específicas
    ...
    while (!isLast) {
      ...
      const result = await fetch(`${effectiveBaseUrl}/rest/api/3/search/jql`, {
        method: 'POST',
        headers: {
          'Authorization': authHeader,
          'Accept': 'application/json',
