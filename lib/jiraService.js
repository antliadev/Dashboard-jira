...
if (!response.ok) {
      ...
      // Testa a JQL com projetos específicos - formato correto
      const jqlToTest = (jql && jql.trim()) || DEFAULT_JQL;
      console.log('[JiraService] Testando JQL:', jqlToTest);
      
      // Usar o endpoint correto /search/jql com campos
      const jqlResponse = await fetch(`${baseUrl}/rest/api/3/search/jql`, {
        ...
        body: JSON.stringify({
          ...
          maxResults: 100,
          startAt: 0,
          fields: ['key', 'summary', 'status', 'project', 'assignee', 'priority', 'updated', 'created']
        })
      });
      
      let testResult = { total: 0, issues: [] };
      if (jqlResponse.ok) {
        const json = await jqlResponse.json();
        console.log('[JiraService] JQL Response keys:', Object.keys(json));
        console.log('[JiraService] JQL Response total:', json.total);
        console.log('[JiraService] JQL Response issues.length:', json.issues?.length);
        testResult = { total: json.total || 0, issues: json.issues || [] };
      } else {
        const errText = await jqlResponse.text();
        console.log('[JiraService] JQL Error:', jqlResponse.status, errText);
        return { success: false, error: `Erro na busca: ${errText.substring(0, 100)}` };
      }
...
/**
   ...
   * CORRIGIDO: conforme especificações do usuário
   */
  async fetchAllIssuesWithConfig(baseUrl, email, token, jql = null) {
    ...
    console.log('[JiraService] URL:', baseUrl);
    ...
    // Loop de paginação usando startAt
    do {
      const payload = {
        jql: effectiveJql,
      ...
      console.log('[JiraService]Requisição:', JSON.stringify(payload));
      ...
      if (!result.ok) {
        ...
        throw new Error(`Erro do Jira (${result.status}): ${errorText.substring(0, 500)}`);
      }
      
      const json = await result.json();
      const issues = json.issues || [];
      
      console.log('[JiraService] Resposta:');
      console.log('[JiraService]   total:', json.total);
      console.log('[JiraService]   issues.length:', issues.length);
      console.log('[JiraService]   isLast:', json.isLast);
      console.log('[JiraService]   nextPageToken:', json.nextPageToken ? 'sim' : 'não');
      ...
      console.log(`[JiraService] Página ${pageCount}: ${issues.length} issues (${allIssues.length}/${total})`);
      
    } while (allIssues.length < total && allIssues.length > 0);
    ...
    if (allIssues.length === 0) {
      throw new Error('Nenhum ticket encontrado. Verifique a JQL e permissoes.');
    }
