...
const docwiseCapacityHours = Math.max(1, Number(process.env.DOCWISE_MONTHLY_CAPACITY_HOURS) || 100);
export const DOCWISE_MONTHLY_CAPACITY_SECONDS = docwiseCapacityHours * 60 * 60;
const HOURS_PROJECTS = Object.freeze({
  CRAWFORD: { key: 'CRAWFORD', name: 'Crawford', capacitySeconds: CRAWFORD_MONTHLY_CAPACITY_SECONDS },
  DOCW: { key: 'DOCW', name: 'Docwise', capacitySeconds: DOCWISE_MONTHLY_CAPACITY_SECONDS }
});
...
export function buildProjectHoursDashboard(worklogs, issues, selectedCompetence = null, projectKey = 'CRAWFORD') {
  ...
  const state = capacityStatus(selectedSeconds, projectConfig.capacitySeconds);
  ...
  const monthlyConsumption = availableCompetences.map(month => {
    ...
    const monthState = capacityStatus(seconds, projectConfig.capacitySeconds);
    return {
      competence: month,
