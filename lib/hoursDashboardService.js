...
export async function fetchCrawfordHoursDashboard({ competence = null } = {}) {
  ...
  if (worklogError) throw new Error(`Erro ao ler worklogs Crawford: ${worklogError.message}. Execute sql/migration-hours-dashboard.sql.`);
  if (issueError) throw new Error(`Erro ao ler tickets Crawford: ${issueError.message}`);
  return buildCrawfordHoursDashboard(worklogs || [], issues || [], validCompetence);
}
