...
class ConfigService {
  ...
  getConfig() {
    const maskedEmail = this._config.email 
      ? this._config.email.replace(/(.{2})(.*)(@.*)/, '$1***$3') 
      : '';
    
    // Em produção, fullEmail só se source='missing' (não há env vars)
    // mas quandoSalvar no formulário, precisa retornar
    const showFullData = !IS_PRODUCTION || this._source === 'missing';
    
    return {
      baseUrl: this._config.baseUrl,
      email: maskedEmail,
      fullEmail: showFullData ? this._config.email : undefined,
  ...
  setConfig({ baseUrl, email, token, jql, cacheTtlMinutes }) {
    // Em produção, pode editar a menos que JIRA_LOCK_CONFIG=1
    if (IS_PRODUCTION && process.env.JIRA_LOCK_CONFIG === '1') {
      throw new Error('Configuração em produção está bloqueada. Use variáveis de ambiente na Vercel.');
    }
