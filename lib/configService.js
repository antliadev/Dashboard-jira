...
getConfig() {
    ...
    // Em produção, sempre permitir edição via formulário (usuário pode configurar)
    // A menos que JIRA_LOCK_CONFIG=1
    const isLocked = process.env.JIRA_LOCK_CONFIG === '1';
    const canEditValue = !isLocked;
    const showFullData = !IS_PRODUCTION || this._source === 'missing' || this._source === 'none' || canEditValue;
