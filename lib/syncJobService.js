...
export async function getSyncJobStatus(jobId = null) {
  ...
  // Watchdog: se o job está 'running' há mais de 10 minutos, resetar
  if (job && job.status === 'running') {
    const startedAt = job.started_at || job.startedAt || job.created_at || job.createdAt;
    const now = Date.now();
    const startedMs = startedAt ? new Date(startedAt).getTime() : now;
    const elapsedMin = (now - startedMs) / 60000;

    if (elapsedMin > 10) {
      console.warn(`[sync-job:${job.id}] Job running ha ${elapsedMin.toFixed(1)}min (>10min). Resetando para error.`);
      await patchJob(job.id, {
        status: 'error',
        error: 'Processo interrompido por watchdog: sync excedeu o limite de 10 minutos.',
        finishedAt: new Date().toISOString()
      });
      // Re-ler o job após reset
