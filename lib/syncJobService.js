 ...
 * Segunda a sexta-feira, a cada 30 minutos, das 06:00 as 18:00 (Horario de Brasilia).
 */
export function isWithinAutoSyncSchedule(date = new Date()) {
  try {
    ...
    const isWithinHours = (hour >= 6 && hour < 18) || (hour === 18 && minute === 0);

    return isWeekday && isWithinHours && isHalfHourSlot;
  } catch {
    return false;
  }
}
...
export async function executeAutoSync(source = 'cron', { forceScheduleCheck = false } = {}) {
  if (forceScheduleCheck && !isWithinAutoSyncSchedule()) {
    return {
      success: true,
      skipped: true,
      reason: 'Fora dos intervalos de 30 minutos entre 06:00 e 18:00, de segunda a sexta-feira (Horario de Brasilia).'
    };
  }
