...
import { syncCrawfordWorklogs } from './jiraWorklogService.js';
...
export async function runSyncJob(jobId, credentials = null) {
  ...
  try {
    ...
    await appendLog(jobId, 'Sincronizando apontamentos de horas do projeto Crawford.');
    try {
      const worklogResult = await syncCrawfordWorklogs(rawIssues, safeCredentials, {
        onProgress: (message) => appendLog(jobId, message)
      });
      await appendLog(
        jobId,
        `${worklogResult.worklogs} worklogs Crawford sincronizados em ${worklogResult.issues} tickets.`
      );
    } catch (worklogError) {
      ...
      await appendLog(jobId, `Aviso: tickets salvos, mas os worklogs Crawford nao foram atualizados: ${cleanError(worklogError)}`);
    }
...
/**
 * Verifica se a data atual esta dentro do horario permitido para sincronizacao automatica:
 * Segunda a sexta-feira, das 06:00 as 18:00 (Horario de Brasilia).
 */
export function isWithinAutoSyncSchedule(date = new Date()) {
  try {
    ...
    const isWithinHours = hour >= 6 && hour <= 18;
...
export async function executeAutoSync(source = 'cron', { forceScheduleCheck = false } = {}) {
  if (forceScheduleCheck && !isWithinAutoSyncSchedule()) {
    return {
      success: true,
      skipped: true,
      reason: 'Fora do horario agendado (06:00 as 18:00, segunda a sexta-feira, Horario de Brasilia).'
    };
  }
